...
3. 确保代码符合项目的编码规范（ESLint + Prettier），并在提交前运行 `npm run lint` 与 `npm run typecheck`
4. 添加必要的测试和文档
...
代码风格遵循 ESLint 和 Prettier 配置，提交前请运行 `npm run lint` 和 `npm run typecheck`。组件采用函数式组件和 Hooks 模式编写。样式类名使用 TailwindCSS 的原子化类。提交信息使用英文，遵循 Conventional Commits 格式。
